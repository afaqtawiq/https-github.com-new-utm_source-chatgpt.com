...
def create_session(user_id,hours=12):
    ...
    exp=utcnow()+datetime.timedelta(hours=hours)
...
def get_session(sid):
    ...
    with db() as c:
        r=c.execute('SELECT s.*,u.email,u.name,u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id=%s AND s.expires_at>%s',(sid,utcnow())).fetchone()
...
def authenticate(email,password):
    with db() as c:
        r=c.execute('SELECT * FROM users WHERE email=%s',(email,)).fetchone()
...
def one(sql,args=()):
    with db() as c:
        r=c.execute(_sql(sql),args).fetchone(); return dict(r) if r else None
