...
def init_db(admin_email,admin_password):
    ...
    '''CREATE TABLE IF NOT EXISTS inbound_replies(id BIGSERIAL PRIMARY KEY,gmail_message_id TEXT UNIQUE NOT NULL,gmail_thread_id TEXT,opportunity_id BIGINT REFERENCES opportunities(id) ON DELETE SET NULL,matched_outbound_id BIGINT REFERENCES outbound_messages(id) ON DELETE SET NULL,sender TEXT,subject TEXT,snippet TEXT,body_text TEXT,classification TEXT NOT NULL DEFAULT 'unclassified',confidence INTEGER NOT NULL DEFAULT 0,status TEXT NOT NULL DEFAULT 'new',received_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL,updated_at TIMESTAMPTZ NOT NULL)''',
    '''CREATE TABLE IF NOT EXISTS sales_response_actions(id BIGSERIAL PRIMARY KEY,inbound_reply_id BIGINT UNIQUE NOT NULL REFERENCES inbound_replies(id) ON DELETE CASCADE,opportunity_id BIGINT NOT NULL REFERENCES opportunities(id) ON DELETE CASCADE,action_type TEXT NOT NULL,requirements TEXT,suggested_response TEXT,proposal_notes TEXT,outbound_message_id BIGINT REFERENCES outbound_messages(id) ON DELETE SET NULL,status TEXT NOT NULL DEFAULT 'prepared',created_by BIGINT,created_at TIMESTAMPTZ NOT NULL,updated_at TIMESTAMPTZ NOT NULL)''',
    '''CREATE TABLE IF NOT EXISTS sales_quotes(id BIGSERIAL PRIMARY KEY,opportunity_id BIGINT NOT NULL REFERENCES opportunities(id) ON DELETE CASCADE,response_action_id BIGINT REFERENCES sales_response_actions(id) ON DELETE SET NULL,quote_number TEXT UNIQUE NOT NULL,status TEXT NOT NULL DEFAULT 'draft',service_type TEXT NOT NULL,origin TEXT,destination TEXT,cargo_description TEXT,quantity TEXT,currency TEXT NOT NULL DEFAULT 'SAR',base_cost DOUBLE PRECISION NOT NULL DEFAULT 0,margin_percent DOUBLE PRECISION NOT NULL DEFAULT 0,sell_price DOUBLE PRECISION NOT NULL DEFAULT 0,valid_until DATE,assumptions TEXT,exclusions TEXT,created_by BIGINT,created_at TIMESTAMPTZ NOT NULL,updated_at TIMESTAMPTZ NOT NULL)''']
    with db() as c:
