...
def pricing(qid:int,request:Request):
    auth(request);q=calc(qid);o=one('SELECT company_name FROM opportunities WHERE id=?',(q['opportunity_id'],));items=rows('SELECT * FROM quote_cost_items WHERE quote_id=? ORDER BY id',(qid,));approval=one('SELECT * FROM approvals WHERE id=?',(q.get('pricing_approval_id'),)) if q.get('pricing_approval_id') else None
    ...
    if q['requires_manager_approval']:
        ...
        elif approval and approval['status']=='pending':gate='<div class="card warn">بانتظار موافقة الإدارة على هامش أقل من الحد الأدنى.</div>'
    ...
    b=nav()+'<h1>التسعير والهوامش — '+esc(q['quote_number'])+'</h1><p>'+esc((o or {}).get('company_name'))+'</p><div class="grid"><div class="card"><b>إجمالي التكلفة</b><h2>'+format(q['base_cost'],',.2f')+' '+esc(q['currency'])+'</h2></div><div class="card"><b>الهامش</b><h2>'+format(q['margin_percent'],'.2f')+'%</h2><span class="muted">الحد الأدنى '+format(q['min_margin_percent'],'.2f')+'%</span></div><div class="card"><b>قبل الضريبة</b><h2>'+format(q['total_before_vat'],',.2f')+'</h2></div><div class="card"><b>VAT</b><h2>'+format(q['vat_amount'],',.2f')+'</h2></div><div class="card"><b>الإجمالي</b><h2>'+format(q['total_with_vat'],',.2f')+' '+esc(q['currency'])+'</h2></div></div>'+gate
    b+='<div class="card"><h2>بنود التكلفة</h2><form method="post" action="/quotes/'+str(qid)+'/cost"><div class="grid"><select name="item_type"><option>تخليص جمركي</option><option>نقل</option><option>شحن</option><option>مناولة</option><option>تخزين</option><option>رسوم طرف ثالث</option><option>أخرى</option></select><input name="description" placeholder="وصف البند"><input name="quantity" type="number" min="0" step="0.01" value="1" required><input name="unit_cost" type="number" min="0" step="0.01" placeholder="تكلفة الوحدة" required></div><button class="btn">إضافة بند تكلفة</button></form><table><tr><th>البند</th><th>الوصف</th><th>الكمية</th><th>تكلفة الوحدة</th><th>الإجمالي</th><th></th></tr>'+trs+'</table></div>'
