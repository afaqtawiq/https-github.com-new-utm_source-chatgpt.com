...
from app.discovery import fetch_public,run_discovery_cycle,start_discovery_worker
...
async def login(r:Request):
 ...
 sid,csrf,exp=create_session(u['id']);resp=RedirectResponse(safe_next(d.get('next')),303);resp.set_cookie('gla_session',sid,httponly=True,secure=os.getenv('BROWSER_COOKIE_SECURE','1')=='1',samesite='lax',max_age=43200);log(u['id'],'login',summary='Browser login');return resp
@app.get('/logout')
...
def dashboard(r:Request):
 ...
 actions=[]
 if pending:actions.append(f'<a class="action-item" href="/approvals">اعتمادات تنتظر القرار <b>{pending}</b></a>')
 if exceptions:actions.append(f'<a class="action-item" href="/control-tower">استثناءات تشغيلية مفتوحة <b>{exceptions}</b></a>')
 if overdue:actions.append(f'<a class="action-item" href="/control-tower">مهام تشغيل متأخرة <b>{overdue}</b></a>')
 if drafts:actions.append(f'<a class="action-item" href="/outbound">رسائل تحتاج مراجعة <b>{drafts}</b></a>')
 if missing_routes:actions.append(f'<a class="action-item" href="/shipments">شحنات تحتاج تحديد المسار <b>{missing_routes}</b></a>')
 if maps_setup:actions.append('<a class="action-item" href="/discovery">مطلوب ربط Google Maps <b>1</b></a>')
 if search_setup:actions.append('<a class="action-item" href="/discovery">مطلوب ربط محرك البحث <b>1</b></a>')
 total=pending+exceptions+overdue+drafts+missing_routes+maps_setup+search_setup
 action_board=('<div class="action-board"><div class="action-head"><h2>⚠ مطلوب من الإدارة الآن</h2><span class="action-total">'+str(total)+'</span></div><div class="action-list">'+''.join(actions)+'</div></div>') if actions else '<div class="action-board action-clear"><div class="action-head"><h2>✓ لا توجد تدخلات عاجلة مطلوبة من الإدارة</h2><span class="action-total">0</span></div></div>'
 b=head(s,'لوحة التحكم')+action_board+f'<div class="hero"><div class="eyebrow">مركز قيادة العمليات</div><h2>صباح الخير، هذه صورة أعمال آفاق طويق الآن</h2><p class="muted">تابع الشحنات والفرص وابدأ أهم الإجراءات اليومية من لوحة واحدة.</p><div class="quick"><a href="/freight-workflow">🚛 إدارة عروض الشحن<br><span class="muted">التفاوض والإرسال للسائقين</span></a><a href="/shipping-agents">⚓ وكلاء الملاحة<br><span class="muted">البوالص والتأكيدات والمتابعة</span></a><a href="/customer-campaigns">✉ حملات العملاء<br><span class="muted">البروشور والبريد وواتساب</span></a><a href="/commands">🎙 مساعد الأوامر<br><span class="muted">نفّذ مهامك بلغة واضحة</span></a><a href="/content-center">✦ صناعة ونشر المحتوى<br><span class="muted">المسودات والمنصات وجدولة النشر</span></a><a href="/approvals">✓ مركز الموافقات<br><span class="muted">راجع قبل أي إجراء خارجي</span></a></div></div><div class="grid"><div class="kpi">العملاء<b>{a}</b><span class="muted">جهة مسجلة</span></div><div class="kpi"><a href="/drivers" style="color:inherit;text-decoration:none">السائقون<b>{d}</b><span class="muted">سائق في النظام</span></a></div><div class="kpi">الفرص<b>{o}</b><span class="muted">فرصة تجارية</span></div><div class="kpi">إشارات السوق<b>{sig}</b><span class="muted">إشارة مكتشفة</span></div><div class="kpi">مصادر المراقبة<b>{src}</b><span class="muted">مصدر نشط</span></div><div class="kpi">قيمة الفرص<b>{pipe:,.0f}</b><span class="muted">ريال سعودي</span></div></div><div class="card notice"><b>منصة العمليات متاحة.</b> <a href="/readiness">راجع جاهزية القنوات واختبارات التشغيل</a>. البيانات محفوظة في قاعدة مركزية مشتركة، وجميع الإجراءات الخارجية تمر عبر موافقة بشرية.</div><div class="card warn"><b>طلبات واتساب.</b> تابع الطلبات والردود وحالة جمع المتطلبات من صفحة طلبات واتساب.</div>'
...
def discovery(r:Request):
 ...
 f='<div class="hero"><div class="eyebrow">AFAQ OPPORTUNITY RADAR</div><h2>رادار فرص التخليص والنقل والتخزين</h2><p class="muted">يفحص المصادر العامة، يقيم نية الشراء، يمنع التكرار، ويجهز فرصة ومسودة تواصل وموافقة إدارية. لا يرسل أي رسالة دون اعتماد.</p><form method="post" action="/discovery/run"><button class="btn">تشغيل البحث الآن</button></form></div><div class="card"><h2>إضافة مصدر عام للمراقبة</h2><form method="post" action="/discovery/source"><div class="formgrid"><input name="name" placeholder="اسم المصدر" required><input name="url" placeholder="https://..." required></div><button class="btn">إضافة المصدر</button></form></div>'
 st=''.join(f'<tr><td>{esc(x["name"])}</td><td>{esc(x["url"])}</td><td>{esc(x["last_status"])}</td><td>{esc(x["last_checked_at"])}</td></tr>' for x in src); sg=''.join(f'<tr><td>{x["score"]}</td><td>{esc(x["company_name"])}</td><td>{esc(x["title"])}</td><td>{esc(x["matched_terms"])}</td><td>'+('<a class="btn" href="/sales-copilot/'+str(x["opportunity_id"])+'">معالجة الفرصة</a>' if x.get("opportunity_id") else ('<span class="pill">عميل للتأهيل</span>' if str(x["url"]).startswith("internal://") else '<a class="btn" href="'+esc(x["url"])+'" target="_blank">المصدر</a>'))+'</td></tr>' for x in sig)
...
def run_discovery_now(r:Request,tasks:BackgroundTasks):
 s=require(r)
 if not s:return rl()
 tasks.add_task(run_discovery_cycle)
 log(s['user_id'],'run','discovery',summary='تشغيل دورة بحث فرص التخليص والنقل والتخزين')
