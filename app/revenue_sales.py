...
def sales_center(request:Request):
 auth(request)
 total=one("SELECT COUNT(*) n FROM opportunities WHERE stage<>'test'")['n'];hot=one("SELECT COUNT(*) n FROM opportunities WHERE score>=70 AND stage NOT IN ('won','lost','test')")['n'];pending=one("SELECT COUNT(*) n FROM outbound_messages WHERE status='pending_approval'")['n'];approved=one("SELECT COUNT(*) n FROM outbound_messages WHERE status='approved'")['n'];sent=one("SELECT COUNT(*) n FROM outbound_messages WHERE status='sent'")['n'];won=one("SELECT COUNT(*) n FROM opportunities WHERE stage='won'")['n']
 data=rows("SELECT o.id,o.company_name,o.score,o.stage,o.signal,i.priority,i.intent,i.services,i.next_action,m.id message_id,m.status message_status,m.sent_at FROM opportunities o LEFT JOIN opportunity_intelligence i ON i.opportunity_id=o.id LEFT JOIN LATERAL (SELECT * FROM outbound_messages x WHERE x.opportunity_id=o.id ORDER BY x.id DESC LIMIT 1) m ON true WHERE o.stage<>'test' ORDER BY o.score DESC,o.id DESC LIMIT 100")
 trs=''.join('<tr><td>'+esc(x['company_name'])+'</td><td>'+str(x['score'])+'</td><td>'+esc(x.get('priority'))+'</td><td>'+esc(x['stage'])+'</td><td>'+esc(x.get('services'))+'</td><td>'+esc(x.get('message_status') or 'لا توجد')+'</td><td>'+('<a class="btn" href="/outbound/'+str(x['message_id'])+'">فتح الرسالة</a>' if x.get('message_id') else '<a class="btn" href="/sales-copilot/'+str(x['id'])+'">تجهيز البيع</a>')+'</td></tr>' for x in data)
 b=nav()+'<h1>مركز مبيعات آفاق طويق</h1><p class="muted">من إشارة الشراء إلى العرض المعتمد والإرسال والمتابعة.</p><div class="grid"><div class="k">الفرص<b>'+str(total)+'</b></div><div class="k">فرص ساخنة 70+<b>'+str(hot)+'</b></div><div class="k">بانتظار الموافقة<b>'+str(pending)+'</b></div><div class="k">جاهزة للإرسال<b>'+str(approved)+'</b></div><div class="k">رسائل مرسلة<b>'+str(sent)+'</b></div><div class="k">صفقات فائزة<b>'+str(won)+'</b></div></div><div class="card scroll"><table><tr><th>الجهة</th><th>Score</th><th>الأولوية</th><th>المرحلة</th><th>الخدمات</th><th>التواصل</th><th>الإجراء</th></tr>'+trs+'</table></div>'
 return page(b)
...
def api(request:Request):
 auth(request);return {'funnel':rows("SELECT stage,COUNT(*) count,COALESCE(SUM(estimated_value),0) value FROM opportunities WHERE stage<>'test' GROUP BY stage ORDER BY stage"),'outbound':rows("SELECT status,COUNT(*) count FROM outbound_messages GROUP BY status ORDER BY status")}
