...
def action_label(kind):
    return {"call": "مكالمة", "whatsapp": "واتساب", "email": "بريد إلكتروني"}.get(kind, kind)
...
<div class="card examples"><b>أمثلة:</b><br>«اتصل على محمد»<br>«أرسل واتساب إلى شركة النور بخصوص عرض النقل»<br>«أرسل بريدًا إلى أحمد بخصوص خدمات التخليص»<br>«افتح السائقين»</div>
...
def command_api(request: Request):
    session(request)
    return {"automatic_external_actions": False, "supported": ["call", "whatsapp", "email", "navigate"], "items": rows("SELECT id,raw_command,action_type,target_name,recipient,status,created_at FROM command_actions ORDER BY id DESC LIMIT 100")}
