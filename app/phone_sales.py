...
def form(raw):return {k:v[0] for k,v in urllib.parse.parse_qs(raw.decode()).items()}
def sess(r):
...
async def prepare(r:Request):
 ...
 phone=(d.get('phone') or '').strip();objective=(d.get('objective') or '').strip()
 if not phone.startswith('+') or len(phone)<8 or not objective:raise HTTPException(400,'Valid E.164 phone and objective required')
 # Never invent a contact: phone must already exist on this opportunity and be verified.
 contact=one('SELECT id,name,verified FROM sales_contacts WHERE opportunity_id=? AND phone=?',(oid,phone))
