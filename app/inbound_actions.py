...
def build_action(reply,op,intel):
 ...
 if cls=='quote_request':
  ...
  notes='لا يتم إدخال سعر تلقائي. يجب مراجعة النطاق والتكلفة التشغيلية واعتماد العرض قبل الإرسال.'
  action='prepare_quote'
 elif cls=='meeting_request':
  req=common+'\n\nالمطلوب: تحديد هدف الاجتماع، المشاركين، والوقت المناسب.'
...
def prepare(rid:int,request:Request):
 ...
 execute('UPDATE inbound_replies SET status=?,updated_at=? WHERE id=?',('action_prepared',now,rid))
 due=now+datetime.timedelta(days=1);execute('INSERT INTO sales_followups(opportunity_id,kind,due_at,status,notes,created_by,created_at) VALUES(?,?,?,?,?,?,?)',(r['opportunity_id'],'inbound_action',due,'open','مراجعة رد العميل وتجهيز الإجراء/العرض المقترح.',s['user_id'],now))
 log(s['user_id'],'prepare_inbound_sales_action','sales_response_action',aid,'Prepared action and outbound draft; no external send');return RedirectResponse('/sales-action/'+str(aid),303)
