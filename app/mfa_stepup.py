...
def mfa(r:Request):
 ...
 else:body+='<div class="card"><h2>تفعيل المصادقة الثنائية</h2><form method="post" action="/mfa/setup"><input type="hidden" name="csrf" value="'+e(s['csrf'])+'"><button class="btn">بدء الإعداد</button></form></div>'
 return page(body)
@router.post('/mfa/setup')
async def setup(r:Request):
 s=sess(r);d=parse(await r.body())
 if d.get('csrf')!=s['csrf']:raise HTTPException(403)
 secret=gen_secret();now=utcnow();execute('INSERT INTO user_mfa(user_id,secret_enc,mfa_enabled,enrolled_at,updated_at) VALUES(?,?,0,NULL,?) ON CONFLICT(user_id) DO UPDATE SET secret_enc=excluded.secret_enc,mfa_enabled=0,enrolled_at=NULL,updated_at=excluded.updated_at',(s['user_id'],enc(secret),now));uri='otpauth://totp/Gulf%20Logistics%20AI:'+urllib.parse.quote(s['email'])+'?secret='+secret+'&issuer=Gulf%20Logistics%20AI&digits=6&period=30';return page('<h1>إعداد MFA</h1><div class="card"><div class="secret">'+e(secret)+'</div><div class="secret">'+e(uri)+'</div><form method="post" action="/mfa/enable"><input type="hidden" name="csrf" value="'+e(s['csrf'])+'"><input name="code" maxlength="6" required><button class="btn">تأكيد وتفعيل MFA</button></form></div>')
