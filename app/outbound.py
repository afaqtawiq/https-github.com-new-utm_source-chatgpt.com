import os,html,urllib.parse,smtplib
...
def nav(): return '<div class="nav"><a href="/dashboard">الرئيسية</a><a href="/sales-copilot">Sales Copilot</a><a href="/outbound">العروض والرسائل</a><a href="/approvals">الموافقات</a><a href="/activity">السجل</a></div>'

def smtp_ready(): return all(os.getenv(k) for k in ('SMTP_HOST','SMTP_PORT','SMTP_FROM')) and os.getenv('ENABLE_EXTERNAL_ACTIONS','0')=='1'
def send_email(m):
    if not smtp_ready(): raise RuntimeError('قناة البريد غير مفعلة بعد')
    msg=EmailMessage();msg['From']=os.environ['SMTP_FROM'];msg['To']=m['recipient'];msg['Subject']=m['subject'];msg.set_content(m['body']+'\n\n'+(m.get('proposal_text') or ''))
    host=os.environ['SMTP_HOST'];port=int(os.environ['SMTP_PORT']);user=os.getenv('SMTP_USER');pw=os.getenv('SMTP_PASSWORD');tls=os.getenv('SMTP_STARTTLS','1')=='1'
...
def home(request:Request):
    auth(request);data=rows('''SELECT m.*,o.company_name FROM outbound_messages m JOIN opportunities o ON o.id=m.opportunity_id ORDER BY m.id DESC''')
    trs=''
    for m in data:
        action='<a class="btn" href="/outbound/'+str(m['id'])+'">فتح</a>'
        trs+='<tr><td>'+esc(m['company_name'])+'</td><td>'+esc(m['channel'])+'</td><td>'+esc(m['recipient'])+'</td><td>'+esc(m['status'])+'</td><td>'+action+'</td></tr>'
    state='مفعّل' if smtp_ready() else 'غير مفعّل'
    return HTMLResponse(shell('العروض والرسائل',nav()+'<h1>العروض والرسائل</h1><div class="card"><b>المسار:</b> إنشاء → مراجعة → طلب موافقة → موافقة بشرية → إرسال.<br><span class="muted">الإرسال الخارجي: '+state+'</span></div><div class="card scroll"><table><tr><th>الجهة</th><th>القناة</th><th>المستلم</th><th>الحالة</th><th></th></tr>'+trs+'</table></div>'))

@router.post('/outbound/from-opportunity/{oid}')
async def create_from_opportunity(oid:int,request:Request):
...
def detail(mid:int,request:Request):
    auth(request);m=one('''SELECT m.*,o.company_name FROM outbound_messages m JOIN opportunities o ON o.id=m.opportunity_id WHERE m.id=?''',(mid,))
    ...
    elif m['status']=='pending_approval': edit='<div class="card">بانتظار الموافقة البشرية.</div>'
    ...
    return HTMLResponse(shell('رسالة',nav()+'<h1>'+esc(m['company_name'])+'</h1><div class="card"><b>الحالة:</b> '+esc(m['status'])+'</div>'+edit))
...
def send(mid:int,request:Request):
    ...
    try:
        provider_id=send_email(m);now=utcnow();execute('UPDATE outbound_messages SET status=?,provider=?,provider_message_id=?,sent_at=?,updated_at=?,last_error=NULL WHERE id=?',('sent','smtp',provider_id,now,now,mid));log(s['user_id'],'send_approved_message','outbound_message',mid,'Approved message sent via configured SMTP')
    except Exception as e:
        execute('UPDATE outbound_messages SET last_error=?,updated_at=? WHERE id=?',(str(e)[:500],utcnow(),mid));raise HTTPException(503,str(e))
    return RedirectResponse('/outbound/'+str(mid),303)

@router.get('/api/v7/outbound')
def api(request:Request):
    auth(request);return {'external_actions_enabled':os.getenv('ENABLE_EXTERNAL_ACTIONS','0')=='1','smtp_ready':smtp_ready(),'messages':rows('SELECT id,opportunity_id,channel,recipient,subject,status,approval_id,provider,sent_at,last_error FROM outbound_messages ORDER BY id DESC')}
