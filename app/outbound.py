...
def send(mid:int,request:Request):
 ...
 except Exception as e:execute('UPDATE outbound_messages SET last_error=?,updated_at=? WHERE id=?',(str(e)[:500],utcnow(),mid));raise HTTPException(503,'تعذر الإرسال عبر Gmail')
 return RedirectResponse('/outbound/'+str(mid),303)
@router.get('/api/v7/outbound')
def api(request:Request):
 s=auth(request);c=connection(s['user_id']);return {'external_actions_enabled':os.getenv('ENABLE_EXTERNAL_ACTIONS','0')=='1','gmail_connected':bool(c and c.get('status')=='connected'),'messages':rows('SELECT id,opportunity_id,channel,recipient,subject,status,approval_id,provider,sent_at,last_error FROM outbound_messages ORDER BY id DESC')}
