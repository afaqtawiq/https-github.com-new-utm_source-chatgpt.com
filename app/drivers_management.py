...
def drivers(request: Request, q: str = '', availability: str = '', consent: str = '', page_number: int = 1):
    ...
    query = urllib.parse.urlencode({'q': q, 'availability': availability})
    table_rows = ''.join(
        '<tr><td>'+esc(x['driver_name'])+'</td><td class="ltr">'+esc(x['whatsapp_phone'])+'</td><td>'+esc(x['vehicle_type'])+'</td><td>'+esc(x['current_city'])+'</td><td>'+esc(x['company_name'])+'</td><td>'+esc(x['availability'])+'</td><td><a class="btn" href="/drivers/'+str(x['id'])+'/edit">عرض وتعديل</a></td></tr>'
        for x in data
