...
def playbook(op,z):
    ...
    proposal=('السادة/ '+str(op['company_name'])+'\n\nتحية طيبة،\nيسر آفاق طويق تقديم تصور أولي لخدمات '+services+'.\n\nنقترح مراجعة نطاق العمل والمسارات والكميات والمواعيد المطلوبة، ثم تقديم الحل التشغيلي والتجاري المناسب.\n\nآفاق طويق\nالتخليص الجمركي | النقل | الشحن | التخزين | الباب إلى الباب')
    follow='1) تحقق من المصدر والفرصة.\n2) ثبّت الجهة والبريد من مصدر عام موثوق.\n3) راجع نطاق الخدمة.\n4) أنشئ الرسالة والعرض.\n5) اطلب الموافقة ثم أرسل عبر القناة المفعلة.'
...
def detail(oid:int,request:Request):
    ...
    if not op: raise HTTPException(404)
    z=analyze(op);p=playbook(op,z);intel=one('SELECT * FROM opportunity_intelligence WHERE opportunity_id=?',(oid,));source='<a class="btn" target="_blank" rel="noopener" href="'+esc(op.get('source_url'))+'">فتح المصدر العام</a>' if op.get('source_url') else '—'
    create='<form method="post" action="/outbound/from-opportunity/'+str(oid)+'"><button class="btn">إنشاء رسالة + عرض للإرسال</button></form>' if intel else '<div class="muted">أنشئ ملف البيع أولًا.</div>'
    qualification='<div class="card"><b>تأهيل الطلب:</b> '+esc(z['qualification']['reason'])+'</div>'
    if not z['qualification']['is_request']: create='<div class="card">التواصل التنفيذي متوقف حتى توثيق طلب حقيقي.</div>'
    body=nav()+qualification+'<h1>'+esc(op['company_name'])+'</h1><div class="grid"><div class="card"><span class="pill">'+esc(z['priority'])+'</span><h3>ملاءمة آفاق طويق</h3>'+esc(p['fit'])+'</div><div class="card"><h3>نية الشراء</h3>'+esc(z['intent'])+'</div><div class="card"><h3>Score</h3>'+esc(op['score'])+'/100</div><div class="card"><h3>الخدمات</h3>'+esc('، '.join(z['services']))+'</div></div><div class="card"><h2>عرض القيمة</h2><p>'+esc(p['value'])+'</p></div><div class="card"><h2>الدليل</h2><p>'+esc(z['evidence'])+'</p>'+source+'</div><div class="card"><h2>خطة المندوب</h2><pre>'+esc(p['follow'])+'</pre></div><div class="card"><h2>مسودة العرض التجاري</h2><pre>'+esc((intel or {}).get('proposal_draft') or p['proposal'])+'</pre></div>'+create+'<form method="post" action="/sales-copilot/'+str(oid)+'/prepare"><button class="btn">تحديث ملف البيع</button></form>'
    return HTMLResponse(shell('Sales Copilot',body))

@router.get('/api/v7/sales-copilot')
def api_list(request:Request):
    auth(request);return {'approval_required':True,'opportunities':rows('''SELECT o.id,o.company_name,o.score,o.stage,i.priority,i.intent,i.services,i.next_action,i.follow_up_status FROM opportunities o LEFT JOIN opportunity_intelligence i ON i.opportunity_id=o.id ORDER BY o.score DESC,o.id DESC''')}
