...
with patch.dict(sys.modules, {"app.storage": storage, "app.fine_permissions": perms}):
    for name in ("app.prospects_schema", "app.prospects_qualify", "app.prospects_send"):
        sys.modules.pop(name, None)
    q = importlib.import_module("app.prospects_qualify")
    snd = importlib.import_module("app.prospects_send")
    for name in ("app.prospects_schema", "app.prospects_qualify", "app.prospects_send"):
        sys.modules.pop(name, None)
...
def test_pick_email_prefers_company_role_mailbox():
    ...
    assert q.pick_email(found, "https://www.wahafoods.sa/ar") in {"sales@wahafoods.sa", "info@wahafoods.sa"}
    assert q.pick_email(["owner@gmail.com"], "https://x.sa") == "owner@gmail.com"
    assert q.pick_email(["me@sentry.io", "a@wixpress.com"], "") == ""
...
def test_qualify(name, category, types, tier):
    ...
    assert q.qualify(place, "info@x.sa", True, "+966126000000", "https://x.sa")[1] == tier
...
def test_trading_company_that_also_ships_is_not_a_competitor():
    ...
    assert q.qualify(place, "info@x.sa", True, "+966126000000", "https://x.sa")[1] == "qualified"


def test_closed_and_no_mx():
    assert q.qualify({"displayName": {"text": "مصنع"}, "businessStatus": "CLOSED_TEMPORARILY"})[1] == "excluded"
    assert q.qualify({"displayName": {"text": "مصنع حديد"}}, "info@x.sa", False, "+966126000000", "")[1] != "qualified"


def test_private_hosts_are_never_fetched():
    assert not q._public_host("localhost")
    assert not q._public_host("127.0.0.1")
    assert not q._public_host("169.254.169.254")
...
def test_emails_from_website_follows_contact_page():
    ...
    with patch.object(q, "_fetch", side_effect=lambda url, client: (pages.get(url, ""), url)):
        assert q.emails_from_website("https://acme.sa") == ("info@acme.sa", "https://acme.sa/contact-us")


def test_follow_up_has_no_prices_and_announces_stop():
    subject, body, html_body, attach = snd.compose({"name": "شركة س", "unsubscribe_token": "t"}, 2)
